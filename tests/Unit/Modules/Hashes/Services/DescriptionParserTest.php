...
class DescriptionParserTest extends TestCase
{
    public function test_can_parse_description()
    {
        $parser = new DescriptionParser('
            ...
            TTS KEY*:      IXDEV-7290
            FUNC CHANGES*: Add fix of generation of suffix for func2
            MERGE:         a67be5a14a4c6b69af8f45b7e3323534hsd574f
            DEPENDENCIES:  EXT_SYS_INTERVENANTS
        ');

        $this->assertTrue($parser->hasTags());
        $this->assertEquals(['IXDEV-7266', 'IXDEV-7290'], $parser->getTtsKeys());
        $this->assertEquals(['Add fix of generation of suffix for adresse and tty', 'Add fix of generation of suffix for func2'], $parser->getFuncChanges());
        $this->assertEquals(['Add check on sessions with same adresse'], $parser->getTechChanges());
        $this->assertEquals(['c00be5a14a4c6b69af8f45b7e3390187c7da822a', 'a67be5a14a4c6b69af8f45b7e3323534hsd574f'], $parser->getMerges());
        $this->assertEquals(['G_BU, G_INDIVIDU, EXT_SYS_INTERVENANTS', 'EXT_SYS_INTERVENANTS'], $parser->getDependencies());
